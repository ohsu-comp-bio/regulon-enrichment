from enrichment.regulon_utils import *
from enrichment.expression_utils import *
from enrichment.regulon_utils import dirname
import datetime
...
def quantile_nes_score(regulon, expr, cohort):
...
def format_delta(down_reg_sub, up_reg_sub):
    ...
    up_reg_normed = (up_reg_normed.median(axis = 1) / up_reg_normed.median(axis = 1).max()).fillna(0.0)
    ...
    down_reg_normed = (down_reg_normed.median(axis = 1) / down_reg_normed.median(axis = 1).max()).fillna(0.0)
